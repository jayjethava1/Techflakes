...
//	}
	...
	@Override
	public void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers("/save","/get","/oauth/token").permitAll().and().authorizeRequests().antMatchers("/**").hasAnyAuthority("ROLE_ADMIN", "ROLE_USER").and().authorizeRequests().anyRequest().authenticated();
